                ...
                Center(
                  child: Column(
                    children: <Widget>[
                      ConstrainedBox(
                        constraints:
                            BoxConstraints.tight(Size(double.infinity, 100.0)),
                        child: Listener(
...
//SELECT * FROM SAMPLE_TABLE ORDER BY ROWID ASC LIMIT 1
  ...
  void _updateLocation(PointerEvent details) {
    setState(() {
      ...
      if (statusTouch1 && statusTouch2) {
        ...
        if(delayTime < 400) {delayTime = 400;}
  ...
  Timer timer;
  void startTimer( int lama) { // ganti timer periodically
    isStopped = true;
    durationTimeGraph --;
    isStopped = false;

    // Start the periodic timer which prints something every 1 seconds
    timer = new Timer.periodic(new Duration(milliseconds: lama),
        (time) {
      if (isStopped) {
        timer.cancel();
      }
