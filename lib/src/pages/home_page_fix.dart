...
var Longplusright = 0, Longplusleft = 0, longminright = 0, longminleft = 0;
...
class _HomePageState extends State<HomePage> {
  ...
  void _decreaseCounterWhilePressed() async {
    ...
    void _increaseCounterWhilePressed1() async {
    // make sure that only one loop is active
    if (_loopActive) return;
  ...
  void _decreaseCounterWhilePressed1() async {
    // make sure that only one loop is active
    if (_loopActive) return;
  ...
  void baca_hold(var WaitData) {
    if (statusWaitSeconds > 0) {
      Future.delayed(Duration(seconds: 1), () {
        AddManual += statusWaitSeconds;
        //AddManualLeft -= statusWaitSeconds;
        changeQuery1("data1", "data2");
        print("tambah data dengan onlong" + AddManual.toString());
      });
    }
  }
